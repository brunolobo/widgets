...
namespace Brunolobo\Widgets\Factories;

use Brunolobo\Widgets\AbstractWidget;
use Brunolobo\Widgets\Contracts\ApplicationWrapperContract;
use Brunolobo\Widgets\Misc\EncryptException;
use Brunolobo\Widgets\Misc\InvalidWidgetClassException;
use Brunolobo\Widgets\Misc\ViewExpressionTrait;
...
abstract class AbstractWidgetFactory
{
    ...
    protected function instantiateWidget(array $params = [])
    {
        ...
        $rootNamespace = $this->app->config('pacto-widgets.default_namespace', $this->app->getNamespace().'Widgets');

        $fqcn = $rootNamespace.'\\'.$this->widgetName;
        $widgetClass = class_exists($fqcn) ? $fqcn : $this->widgetName;

        if (!class_exists($widgetClass)) {
            throw new InvalidWidgetClassException('Classe "'.$widgetClass.'" não existe');
        }

        if (!is_subclass_of($widgetClass, 'Brunolobo\Widgets\AbstractWidget')) {
            throw new InvalidWidgetClassException('Classe "'.$widgetClass.'" deve extender a classe "Brunolobo\Widgets\AbstractWidget"');
        }

        $this->widget = new $widgetClass($this->widgetConfig);

        if (static::$allowOnlyWidgetsWithDisabledEncryption && $this->widget->encryptParams) {
            throw new EncryptException('Widget "'.$widgetClass.'" chamado de forma incorreta');
        }
    }
